...
let package = Package(
    ...
    targets: [
        .executableTarget(
            ...
            exclude: [
                // `swift.plist` is not a Swift source file, so it must be excluded
                // from macOS and Linux builds generated by the Swift CLI compiler
                "swift.plist"
            ],
            linkerSettings: [
                // Embed `swift.plist` into the __TEXT,__info_plist Mach-O section so the binary
                // can be signed and notarized. `.unsafeFlags` is fine here because `gitcheck` is
